defmodule <%= inspect context.web_module %>.<%= inspect Module.concat(schema.web_namespace, schema.alias) %>Controller do
  ...
  def index(conn, _params) do
    <%= schema.plural %> = <%= inspect context.alias %>.list_<%= schema.plural %>(tenant)
  ...
  def create(conn, %{<%= inspect schema.singular %> => <%= schema.singular %>_params}) do
    case <%= inspect context.alias %>.create_<%= schema.singular %>(<%= schema.singular %>_params, tenant) do
      {:ok, <%= schema.singular %>} ->
        conn
  ...
  def show(conn, %{"id" => id}) do
    <%= schema.singular %> = <%= inspect context.alias %>.get_<%= schema.singular %>!(id, tenant)
  ...
  def update(conn, %{"id" => id, <%= inspect schema.singular %> => <%= schema.singular %>_params}) do
    ...
    case <%= inspect context.alias %>.update_<%= schema.singular %>(<%= schema.singular %>, <%= schema.singular %>_params, tenant) do
      {:ok, <%= schema.singular %>} ->
        conn
  ...
  def delete(conn, %{"id" => id}) do
    {:ok, _<%= schema.singular %>} = <%= inspect context.alias %>.get_<%= schema.singular %>!(id, tenant) |> <%= inspect context.alias %>.delete_<%= schema.singular %>(tenant)
